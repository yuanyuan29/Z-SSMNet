# This code is adapted from https://github.com/MrGiovanni/ModelsGenesis/blob/master/competition/utils.py. 
...
def nonlinear_transformation(x, prob=0.5, normalisation="minmax"):
    """
    ...
    if normalisation == "z-score":
        x_start, x_end = np.percentile(x[0:3, :, :,:], 0.1), np.percentile(x[0:3, :, :,:], 99.9)
        points = [
            [x_start, x_start], 
    ...
    nonlinear_x = np.interp(x[0:3, :,:,:], xvals, yvals)
    nonlinear_x = np.concatenate([nonlinear_x, x[3:,:,:,:]], axis=0)
    return nonlinear_x
...
class CustomDataset(Dataset):
    def __init__(self, data_dir, config):
        self.imgs = glob.glob(os.path.join(data_dir, "*.npy"))
        self.config = config
    ...
    def __getitem__(self, idx):
        x, y = generate_single_pair(np.load(self.imgs[idx]), config=self.config)
